# Brushless DC Motor Design and Build

<p align="center">
  <a href="https://github.com/DanielOkhman/Daniel-Okhman">
    ← Back to Main Portfolio
  </a>
</p>

---
## Objective
Design and Build a motor to research efficiency

The purpose of this project was to design build and test and electric motor to reach a high level of efficiency. The primary focus was to increase working efficiency and see the application of different changes in the design. This hands-on experience was designed to deepen my understanding of electronics, controllers, and electromagnetism.

---

### Skills Learned/Refined

- Electromagnetic system design and magnetic flux path considerations
- Three-phase motor topology and commutation concepts
- Coil winding techniques and phase configuration
- Arduino-based data acquisition and control integration
- Operation and integration of a 3-phase motor controller
- Phase voltage measurement using oscilloscope diagnostics
- Design in Fusion 360
- Prototyping using 3D printing

---

### Tools Used

- Fusion 360 (CAD design & modeling)
- Arduino IDE
- Oscilloscope measurement software/interface
- 3D printer
- Bench power supply

---

### Steps Taken

#### 1) Design
<table>
<tr>
<td align="center">
<img src="https://github.com/DanielOkhman/Brushless-DC-Motor-Design-and-Build/blob/50c4c99d479dd94dc32e7062dd7d5f688a0712f2/Screenshot%202026-02-28%20121526.png" width="400"/><br>
</td>

<td align="center">
<img src="https://github.com/DanielOkhman/Brushless-DC-Motor-Design-and-Build/blob/50c4c99d479dd94dc32e7062dd7d5f688a0712f2/Screenshot%202026-02-28%20121426.png" width="400"/><br>
</td>
</tr>
</table>
Exploded and assembled CAD views displays motor component integration

All parts achieved an appropriate fit including bearings and shaft alignment.

#### 2) Assembly
<table>
<tr>
<td align="center">
<img src="https://github.com/DanielOkhman/Brushless-DC-Motor-Design-and-Build/blob/6a934d01ce711f70aa0d2500dbf3b0ae6c0a002c/IMG_6542.jpeg" width="400"/><br>
</td>

<td align="center">
<img src="https://github.com/DanielOkhman/Brushless-DC-Motor-Design-and-Build/blob/85935705922fd17cd7e72fb91e2abe28dd3aee59/IMG_6538.jpeg" width="400"/><br>
</td>
</tr>
</table>

  
Bearings and shaft installed for a snug fit

Stator winding done by hand

Poles attached to the rotor alternating polarity

All phases connected and wired to the PWM controller


#### 3) Testing
<table>
<tr>
<td align="center">
<img src="https://github.com/DanielOkhman/Brushless-DC-Motor-Design-and-Build/blob/35539d754dd01564329150c90dc8710dc6e8cc09/IMG_6587.jpeg" width="400"/><br>
</td>

<td align="center">
<img src="https://github.com/DanielOkhman/Brushless-DC-Motor-Design-and-Build/blob/62e56f0ecad665f547eae1bd05087dbd204b755d/Experiment%20Setup%20(1).jpg" width="400"/><br>
</td>
</tr>
</table>

  
Created an experimental setup for optimal testing procedure

Used Arduino Infrared Sensor and slotted disk to measure the RPM

Collected data for post-processing


#### 4) Demo

[Watch Test Video]([media/motor-test.mp4](https://github.com/DanielOkhman/Brushless-DC-Motor-Design-and-Build/blob/62e56f0ecad665f547eae1bd05087dbd204b755d/IMG_6588.mov))


<p align="center">
  <a href="https://github.com/DanielOkhman/Daniel-Okhman">
    ← Back to Main Portfolio
  </a>
</p>

---
